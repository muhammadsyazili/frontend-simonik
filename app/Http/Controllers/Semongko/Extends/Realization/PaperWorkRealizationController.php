...
namespace App\Http\Controllers\Semongko\Extends\Realization;
...
class PaperWorkRealizationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!is_null($request->query('level')) && !is_null($request->query('unit')) && !is_null($request->query('tahun'))) {
            $response = SEMONGKO_services('/realizations/paper-work/edit', 'get', [
        ...
        return view('components.semongko.realization.paper-work.read-new', compact('response'));
    }
    ...
    public function update(Request $request)
    {
        ...
        $response = SEMONGKO_services('/realizations/paper-work', 'put', [
        ...
        return redirect()->route('semongko.realizations.paper-work.index', ['level' => $request->level, 'unit' => $request->unit, 'tahun' => $request->tahun]);
    }
    ...
    public function import(Request $request, $level, $unit, $tahun)
    {
        ...
        $response = SEMONGKO_services('/realizations/paper-work/import', 'put', [
            'level' => $level,
            'unit' => $unit,
            'tahun' => $tahun,
            'realizations' => $realizations,
        ...
        Session::flash('info_message', $response->object()->message);
        unlink(public_path("/files/$fileName"));
        return redirect()->route('semongko.realizations.paper-work.index', ['level' => $level, 'unit' => $unit, 'tahun' => $tahun]);
    }

    public function export($level, $unit, $tahun)
    {
        $response = SEMONGKO_services('/realizations/paper-work/export', 'get', [
            'level' => $level,
            'unit' => $unit,
            'tahun' => $tahun,
        ]);
