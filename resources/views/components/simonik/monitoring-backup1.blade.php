...
@push('script')
    ...
    <script>
        function chart(name, type, measure) {
                                                ...
                                                <tbody id="myTable">
                                                    @foreach ($response->data->indicators as $indicator)
                                                        <tr style="background-color: rgb({{ $indicator->bg_color->r }}, {{ $indicator->bg_color->g }}, {{ $indicator->bg_color->b }}); @if (($indicator->bg_color->r < 127.5) && ($indicator->bg_color->g < 127.5) && ($indicator->bg_color->b < 127.5)) color: white; @endif">
                                                            <td class="small">
                                                                <div class="small" style="width: 300px;">
                                                            ...
                                                            <td class="text-center small">
                                                                @if (!is_null($indicator->status))
                                                                    <button type="button" class="btn btn-sm btn-outline-info" onclick="chart('{{ $indicator->indicator }}', '{{ $indicator->type }}', '{{ $indicator->measure }}')"><i class="fas fa-chart-bar"></i></button>
                                                                @endif
                                                            </td>
